...
<body>
    ...
    <div id="contenido_web">
        ...
        if ($buscar == " ") {
            $consulta = "SELECT * FROM form_dos";
        } else {
            $consulta = "SELECT * FROM form_dos WHERE nombre like '%$buscar%'";
        }
        ...
        ?>
                <div class="card">
                    <h5 class="card-header"><?php echo $row["nombre"] ?></h5>
                    <div class="card-body">
                        <h5 class="card-title"><?php echo "RNC: " . $row["rnc"] ?></h5>
                        <p class="card-text"><?php echo "Identidad pública: " . $row["cnc_identidad"] ?></p>
                        <p class="card-text"><?php echo "Disposición: " . $row["dispone"] ?></p>
                        <p class="card-text"><?php echo "Alcace: " . $row["alcance"] ?></p>
                        <p class="card-text"><?php echo "Dedicación: " . $row["dedicacion"] ?></p>
                        <p class="card-text"><?php echo "Industria: " . $row["industria"] ?></p>
                        <p class="card-text"><?php echo "Tamaño: " . $row["tamano"] ?></p>
                        <p class="card-text"><?php echo "Operaciones: " . $row["operaciones"] ?></p>
                        <adress class="card-text">
                            <abbr title="Phone">Tel Principal: </abbr><?php echo $row["tel_principal"] ?><br>
                            <abbr title="Phone">Tel Directo: </abbr><?php echo $row["tel_directo"] ?><br>
                            <abbr title="Phone">Ext: </abbr><?php echo $row["tel_ext"] ?><br>
                            <abbr title="Phone">Email: </abbr><?php echo $row["correo_contacto"] ?><br>
                            <abbr title="Persona">Contacto: </abbr><?php echo $row["contacto"] ?><br>
                            <abbr title="Email">Email Principal: </abbr><a href="#"><?php echo $row["email"] ?></a><br>
                            <p class="card-text"><?php echo "Clave: " . $row["clave"] ?></p>

                            <abbr title="ubicacion">Ubicación: </abbr><?php echo $row["direccion"] . " " . $row["seccion"] . " " . $row["sector"] . " " . $row["municipio"] . " " . $row["provincia"] ?>
                        </adress>
                    </div>
                    <?php echo "<a href='borrar_empresa.php?id=$row[id_empresa]' class='btn btn-danger btn-lg btn-block'>Borrar Vacante</a>" ?>
                    <?php echo "<a href='editar_empresas.php?id=$row[id_empresa]' class='btn btn-warning btn-lg btn-block'>Editar Vacante</a>" ?>
                </div>
        <?php
            }
        }
        ?>
        <a href="form2.php" class="btn btn-primary btn-lg btn-block">Agregar Empresas</a>
    </div>
    <!-- Footer -->
