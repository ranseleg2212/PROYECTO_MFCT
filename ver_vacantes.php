...
<head>
    ...
    <title>CONSULTAR-VACANTES</title>
    <!-- Bootstrap core CSS -->
...
<body>
    ...
    <div id="contenido_web">
        <form method="post">
            <div class="form-row">
                <div class="form-group col">
                    <input type="text" name="buscar_txt" class="form-control" placeholder="Nombre de la empresa o del puesto">
                </div>
                <div class="form-group col">
        ...
        if($buscar == " "){
            $consulta = "SELECT * FROM form_tres";
        }else{
            $consulta = "SELECT * FROM form_tres WHERE nombre_empresa like '%$buscar%' OR puesto like '%$buscar%'";
        }
        ...
        ?>
                <div class="card">
                    <h5 class="card-header"><?php echo $row["nombre_empresa"] ?></h5>
                    <div class="card-body">
                        <h5 class="card-title"><?php echo "Puesto de trabajo: " . $row["puesto"] ?></h5>
                        <p class="card-text"><?php echo "Funciones del puesto: " . $row["funciones_perfil"] ?></p>
                        <p class="card-text"><?php echo "Horario: " . $row["horario"] ?></p>
                        <p class="card-text"><?php echo "Sueldo: " . $row["sueldo"] ?></p>
                        <p class="card-text"><?php echo "Contrato: " . $row["tipo_contacto"] ?></p>
                        <adress class="card-text">
                        <abbr title="Phone">Tel: </abbr><?php echo $row["telefono"] ?><br>
                        <abbr title="Email">@: </abbr><a href="mailto:#"><?php echo $row["correo_cv"] ?></a><br>
                        <abbr title="Persona">Contacto: </abbr><?php echo $row["contacto_persona"] ?><br>
                        <abbr title="ubicacion">Ubicación: </abbr><?php echo $row["ubicacion"] ?>
                        </adress>
                    </div>
                    <?php echo "<a href='borrar_vacante.php?id=$row[id]' class='btn btn-danger btn-lg btn-block'>Borrar Vacante</a>"?>
                    <?php echo "<a href='editar_vacante.php?id=$row[id]' class='btn btn-warning btn-lg btn-block'>Editar Vacante</a>"?>
                </div>
        <?php
            }
        }
        ?>
        <a href="form3.php" class="btn btn-primary btn-lg btn-block">Agregar Vacantes</a>
    </div>
    <!-- Footer -->
